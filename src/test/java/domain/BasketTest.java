...
class BasketTest {
    ...
    void testGetPrice_givenNoBook_ItReturnsZero() {
        assertEquals(0.0, new Basket().getPrice());
    }

    @org.junit.jupiter.api.Test
    void testGetPrice_givenOnlyOneBookOfTheSeries_ItReturnsMultiplesOfStandardBookPrice() {
        assertEquals(8.0,
                new Basket().getPrice(new Book.ChamberOfSecrets())
        );

        assertEquals(2 * 8.0,
                new Basket().getPrice(new Book.ChamberOfSecrets(), new Book.ChamberOfSecrets())
        );
    }
